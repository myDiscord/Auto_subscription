# Events from the ga_hits.csv file that are considered targets in this task.
target_events = [
...
# Values that are gaps.
missing_values = [float('nan'), '(none)', '(not set)', '0x0']

# Organic traffic sources in sessions['utm_medium'].
organic_mediums = ['organic', 'referral', '(none)']

# Social media IDs as source.
...
# Data obtained using the `workalendar` library. Saved as a list
# for comfort. In a real project, it would be better to get data directly from
# libraries.
russian_holidays = [
...
# Distances obtained using the `geopy` library. Stored them in a dictionary for
# convenience and independence from the Internet connection. In a real project it would be worth
# get real-time distance data to Moscow or save
# data in a more convenient form - to a csv file or database.
...
def get_distance_from_moscow(city: str) -> float:
    """
    Returns the distance from `city` to Moscow in kilometers.
    Returns -1 for unknown and foreign cities.
    """
